...
- **This document’s release:** `v0.4.0` — synthetic **`ps aux`** + session PIDs + `top`/`ps -ef` shared table; see [Tags](#version-tags) and **`RELEASE_NOTES_v0.4.0.md`**
...
| **v0.4.0** | **`ps aux`** is **synthesized** from the same parsed **`ps_aux`** table as **`ps -ef`** / **`top`**, with **session `-bash` / `ps` rows** (emulated PIDs, real TTY), optional **`ps_aux_tail_noise_max`** kworker lines, and a **formatter fix** so `%MEM` and 7-digit **`VSZ`** do not fuse. See **`RELEASE_NOTES_v0.4.0.md`**. |
...
- **`ps aux`:** Built from **`build_session_ps_rows()`** + **`format_ps_aux_output()`** (not a verbatim `ps_aux.txt` dump); aligns with **`ps -ef`** / **`top`** and session **PID/TTY** behavior. Optional **`ps_aux_tail_noise_max`** in **`[shell]`**.
- **Detail:** **`RELEASE_NOTES_v0.4.0.md`** (problem, fix, `%MEM`/`VSZ` column bug). **Cursor:** **`.cursor/rules/scalpel-hackathon.mdc`** (Project SCALPEL hackathon context).
...
- **PID coherence (v0.3.0):** **`ps -ef`** and **`top -bn1`** derived from parsed **`ps_aux`** (`core/ps_coherence.py`). *v0.4.0 extends this to **`ps aux` itself and session rows—see above.*
